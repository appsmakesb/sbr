...
public class RecyclerAdapterCategory extends RecyclerView.Adapter<RecyclerAdapterCategory.MyViewHolder> implements Filterable {
    ...
    public void onBindViewHolder(MyViewHolder holder, final int position) {
        ...
        Transformation transformation = new RoundedTransformationBuilder().cornerRadiusDp(6).oval(false).build();

        Picasso.get().load(var_twoe + "/upload/category/" + category.getCategory_image()).placeholder(R.drawable.ic_loading)
                .resize(250, 250).centerCrop().transform(transformation).into(holder.category_image);
